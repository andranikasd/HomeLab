...
resource "null_resource" "install_and_configure_argocd" {
  depends_on = [
    null_resource.setup_kubectl_bastion  // Ensure kubectl is set up and the cluster is ready
  ]
  
  # Copy the ArgoCD installation script to the master node
  provisioner "file" {
    connection {
      host                = split("/", module.masters.vm_ips[0])[0]
      user                = var.cloud_init_user
  ...
  # Copy the ArgoCD Application YAML manifest to the master node
  provisioner "file" {
    connection {
      host                = split("/", module.masters.vm_ips[0])[0]
      user                = var.cloud_init_user
    ...
    source      = "${path.module}/config/argocd-base.yaml"   // Ensure this file is defined in your module
    destination = "/home/${var.cloud_init_user}/argocd-base.yaml"
  }
  
  # Execute the ArgoCD installation and Application deployment script on the master node
  provisioner "remote-exec" {
    connection {
      host                = split("/", module.masters.vm_ips[0])[0]
      user                = var.cloud_init_user
    ...
    inline = [
      ...
      "sudo /home/${var.cloud_init_user}/install_argocd.sh -c /home/${var.cloud_init_user}/argocd-base.yaml"
    ]
  }
}